...
public class BookingService {
    ...
    public Booking createBooking(BookingRequestDto request, Long userId) {
        ...
        OrderCreatedEvent event = new OrderCreatedEvent(savedBooking.getId(), userId, request.roomId());
        kafkaTemplate.send(ORDER_CREATED_TOPIC, String.valueOf(savedBooking.getId()), event);

        log.info("Booking created [PENDING], event sent to Kafka: {}", event);
